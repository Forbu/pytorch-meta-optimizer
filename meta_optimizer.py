...
import math
from utils_ import preprocess_gradients
from layer_norm_lstm import LayerNormLSTMCell
from layer_norm import LayerNorm1D
from GraphConvLayer import GCN, GCN_double
...
class GraphMetaModel_double(nn.Module):
    def __init__(self, model, num_layers, hidden_size, dropout):
        super(GraphMetaModel_double, self).__init__()
        self.meta_model = model

        # put here the graph neural network
        n_input = 5
        self.graphConv = GCN_double(n_input, hidden_size, 2, dropout)

    def forward(self, x, adj_1,adj_2):
        # Gradients preprocessing
        x = self.graphConv(x, adj_1,adj_2)
        return x.split(1, 1)
    ...
    def meta_update(self, model_with_grads, adj_1,adj_2):
        # First we need to create a flat version of parameters and gradients
        grads = []
        ...
        self.f, self.i = self(inputs, adj_1,adj_2)
